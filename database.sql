...
-- Schema delivery
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `delivery` DEFAULT CHARACTER SET utf8 COLLATE utf8_danish_ci ;
USE `delivery` ;
...
CREATE TABLE IF NOT EXISTS `delivery`.`tb_estabelecimentos` (
  ...
  `enderecoOpcional` VARCHAR(45) NULL,
  `status` VARCHAR(1) NULL,
  `dtCreated` DATETIME default now(),
  `dtUpdated` VARCHAR(45) NULL,
  `idUsuario` INT NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `delivery`.`tb_push` (
  ...
  `idUsuario` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_tb_push_tb_usuarios1_idx` (`idUsuario` ASC) ,
